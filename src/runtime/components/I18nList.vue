<template>
  <div>
    Neu:
    <I18nItem
      @created="addItemToList($event)"
    />
    <div
      v-for="localeCode in Object.keys(i18nData)"
      :key="localeCode"
    >
      <h1 v-text="localeCode" />
      <ul>
        <li
          v-for="(item, index) in i18nData[localeCode]"
          :key="index"
        >
          <I18nItem
            :item="item"
            @deleted="removeItemFromList($event)"
          />
        </li> 
      </ul>
    </div> 
...
import { useHelper } from "../helper";
  ...
  function addItemToList(newItem){
    if(!i18nData.value[newItem.localeCode]) i18nData.value[newItem.localeCode] = []
